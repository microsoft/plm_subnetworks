...
from plm_inv_subnetworks.subnetwork.modules import SubnetworkESM
from plm_inv_subnetworks.utils.metrics import (
    aggregate_over_seq,
    logits_kl,
...
class ESMMaskLearner(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        inv_subnet_logits = self.subnetwork(tgt, inverse=False, masks=masks, return_contacts=False)["logits"]
        inv_subnet_logists_dist = torch.nn.functional.softmax(inv_subnet_logits, dim=-1)
        unif_ref = torch.full_like(esm_logits_dist, 1 / len(self.subnetwork.esm.alphabet.all_toks))
        
        per_res_subnet_unif_kl = logits_kl(inv_subnet_logists_dist, unif_ref, batch["seq_mask"], epsilon=1e-4)
        per_res_subnet_esm_kl = logits_kl(inv_subnet_logists_dist, esm_logits_dist, batch["seq_mask"], epsilon=1e-4)
